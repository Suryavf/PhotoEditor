...
void abrir_imagen(BMP *imagen, char *ruta){
    ...
    imagen->pixelB = (unsigned char**)malloc(((unsigned long)imagen->alto)*sizeof(unsigned char *));
	for( i=0; i<imagen->alto; i++)
        imagen->pixelB[i] = (unsigned char *)malloc(((unsigned long)imagen->ancho)*sizeof(unsigned char));

	//Reservar memoria para el arreglo que tendra la imágen en escala de GREEN (Arreglo de tamaño "img.ancho X img.alto")
    imagen->pixelG = (unsigned char**)malloc(((unsigned long)imagen->alto)*sizeof(unsigned char *));
	for( i=0; i<imagen->alto; i++)
        imagen->pixelG[i] = (unsigned char *)malloc(((unsigned long)imagen->ancho)*sizeof(unsigned char));

	//Reservar memoria para el arreglo que tendra la imágen en escala de RED (Arreglo de tamaño "img.ancho X img.alto")
    imagen->pixelR = (unsigned char**)malloc(((unsigned long)imagen->alto)*sizeof(unsigned char *));
	for( i=0; i<imagen->alto; i++)
        imagen->pixelR[i] = (unsigned char *)malloc(((unsigned long)imagen->ancho)*sizeof(unsigned char));
...
void crear_imagen(BMP *imagen, char ruta[],int escala){
    ...
    imgNew.pixelB = (unsigned char**)malloc(((unsigned long)imgNew.alto)*sizeof(unsigned char *));
	for( i=0; i<imgNew.alto; i++)
        imgNew.pixelB[i] = (unsigned char*)malloc(((unsigned long)imgNew.ancho)*sizeof(char));

	//Reservar memoria para el arreglo que tendra imgNew en escala de GREEN (Arreglo de tamaño "imgNew.ancho X imgNew.alto")
    imgNew.pixelG = (unsigned char**)malloc(((unsigned long)imgNew.alto)*sizeof(char *));
	for( i=0; i<imgNew.alto; i++)
        imgNew.pixelG[i] = (unsigned char*)malloc(((unsigned long)imgNew.ancho)*sizeof(char));

	//Reservar memoria para el arreglo que tendra imgNew en escala de RED (Arreglo de tamaño "imgNew.ancho X imgNew.alto")
    imgNew.pixelR = (unsigned char**)malloc(((unsigned long)imgNew.alto)*sizeof(char *));
	for( i=0; i<imgNew.alto; i++)
        imgNew.pixelR[i] = (unsigned char*)malloc(((unsigned long)imgNew.ancho)*sizeof(char));
